...
file_to_xml_router = app = APIRouter()

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger()

system_prompt = os.getenv("FILE_TO_XML_SYSTEM_PROMPT")

# POST - /api/file_to_xml
@app.post("")
async def file_to_xml(file: UploadFile = File(...), conversion_model_type: str | None = Form(None)):
    try:
        if file.filename == "":
