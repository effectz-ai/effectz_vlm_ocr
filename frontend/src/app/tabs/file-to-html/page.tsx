...
export default function FileToHTML() {
  ...
  const handleApiCall = async (): Promise<string> => {
    ...
    try {
      const {data} = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/file_to_html`, formData);
      return data.html;
    } catch (error) {
      ...
      <div className="min-h-screen bg-gray-100">
        <main className="container mx-auto px-4 py-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              <h2 className="text-xl font-semibold mb-4 text-green-950">HTML Viewer</h2>
              <HTMLViewer onApiCall={handleApiCall}/>
            </div>
          </div>
        </main>
      </div>
